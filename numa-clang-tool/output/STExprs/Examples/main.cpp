...
int main(int argc, char *argv[])
{
 ...
 int opt;
	...
	// std::cout<<endl;
	numa_thread0.resize(2);
	numa_thread1.resize(2);
	regular_thread0.resize(2);
	regular_thread1.resize(2);
	...
	if(DS_name == "array"){
		...
		numa_Stack_init(DS_config, num_DS, prefill_set, percentages);
		if(thread_config == "numa"){
			numa_thread0[0] = new thread_numa<0>(StackTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else if(thread_config== "regular"){
			regular_thread0[0] = new thread(StackTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else{}

		if(thread_config == "numa"){
			for(int i=0; i < num_threads; i++){
				if(numa_thread0[0] == nullptr){
					continue;
				}
				if(numa_thread0[0]->joinable()){
					numa_thread0[0]->join();
				}
			}
			...
			for(int i=0; i < regular_thread0.size(); i++){
				if(regular_thread0[0] == nullptr){
					continue;
				}
				if(regular_thread0[0]->joinable()){
					regular_thread0[0]->join();
				}
			}
		}
		else{}
		...
		numa_Queue_init(DS_config, num_DS, prefill_set, percentages);
		if(thread_config == "numa"){
			numa_thread0[0] = new thread_numa<0>(QueueTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else if(thread_config== "regular"){
			regular_thread0[0] = new thread(QueueTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else{}
		...
		numa_BST_init(DS_config, num_DS, keyspace, crossover);
		if(thread_config == "numa"){
			numa_thread0[0] = new thread_numa<0>(BinarySearchTest, 1, duration, 0, num_DS, num_threads, crossover, keyspace);
		}
		else if(thread_config== "regular"){
			regular_thread0[0] = new thread(BinarySearchTest, 1, duration, 0, num_DS, num_threads, crossover, keyspace);
		}
		else{}
		...
		numa_LL_init(DS_config, num_DS, prefill_set, percentages);
		if(thread_config == "numa"){
			numa_thread0[0] = new thread_numa<0>(LinkedListTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else if(thread_config== "regular"){
			regular_thread0[0] = new thread(LinkedListTest, 1, duration, 0, num_DS, num_threads, crossover);
		}
		else{}
